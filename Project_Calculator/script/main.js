...
const Calculator = {
  ...
  First_Operand: null,
  //This checks wether or not the second operand has been inputted by the use.
...
function Handles_Operator(Next_Operator) {
  ...
  //When an operator key is pressed we convert the current number displayed on the screento a number and than store the result in Calculator.First_Operand if it doesn't already exist.
  const Value_of_Input = parseFloat(Display_Value);
  console.log(typeof Value_of_Input);
  ...
  if (First_Operand == null) {
    Calculator.First_Operand = Calculator.Value_of_Input;
  } else if (Calculator.operator) {
    //Checks if an operator already exists
    const Value_Now = Calculator.First_Operand || 0;
    //If operator exists, property lookup if performed for the operator in the Perform_Calculation object and function that matches the operator is executed.
    let result = Perform_Calculation[operator](
      Calculator.Value_Now,
      Calculator.Value_of_Input
    );
  ...
  Calculator.operator = Calculator.Next_Operator;
}
...
function Update_Display() {
  //Makes use of the calculator-screen class to targer the input tag in the HTML document
